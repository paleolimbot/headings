...
#'   [wmm_ellipsoidal_height()] to convert geoid elevations (EGM9615)
...
wmm_extract <- function(lon, lat, year = wmm_decimal_year(Sys.Date()),
                        height = wmm_ellipsoidal_height(lon, lat, 0), coef = NULL) {
  if (is.null(coef)) {
...
#' @rdname wmm_extract
#' @export
wmm_ellipsoidal_height <- function(lon, lat, height) {
  lon <- vctrs::vec_cast(lon, double())
  lat <- vctrs::vec_cast(lat, double())
  height <- vctrs::vec_cast(height, double())
  ...
  cpp_wmm_ellipsoidal_height(coords, headings::wmm_egm9615_geoid_int)
}
